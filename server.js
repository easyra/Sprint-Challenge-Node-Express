...
server.delete(`/api/actions/:actionId`, (req,res) => {
    ...
    actionModelDb.remove(actionId).then(() => {
        actionModelDb.get().then(actions => {
            res.json(actions)
        })
    })
...
server.delete(`/api/projects/:projectId`, (req,res) => {
    const {projectId} = req.params
    projectModelDb.remove(projectId).then(() => {
        projectModelDb.get().then(projects => {
            res.json(projects)
        })
    })
...
server.put(`/api/actions/:actionId`, (req,res) => {
    const {actionId} = req.params
    const {project_id, description, notes, completed} = req.body;
    actionModelDb.update(actionId, {project_id, description, notes, completed}).then(() =>{
        actionModelDb.get().then(actions => {
            res.json(actions)
        })
    }) 
...
server.put(`/api/projects/:projectId`, (req,res) => {
    const {projectId} = req.params
    const {name, description, completed} = req.body
    projectModelDb.update(projectId, {name, description,completed}).then(() => {
        projectModelDb.get().then(projects => {
            res.json(projects)
        })
    })
